 ...
 * a shared registry dir (~/.pi/mesh/registry). Discovery validates marker
 * identity, then probes the owning process and socket. Crashed peers leave a
 * marker that's pruned on the next read (O(1) per file, no data dirs).
 */
...
import { defaultMeshRegistryDir } from "./types.ts";
...
export function resolveRegistryDir(override?: string): string {
  if (!override) return defaultMeshRegistryDir();
...
export function writeSelfMarker(info: PeerInfo, registryDir: string): void {
  try {
    ...
    // registry write failure is non-fatal (mesh degrades to local-only)
  }
}
