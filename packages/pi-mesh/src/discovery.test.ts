 ...
 * Run: node --test packages/pi-mesh/src/discovery.test.ts
 */
...
beforeEach(() => {
  registryDir = fs.mkdtempSync(path.join(os.tmpdir(), "mesh-registry-"));
});
...
test("resolveRegistryDir expands only leading home shorthand", () => {
  ...
  assert.equal(resolveRegistryDir("~/mesh-registry"), path.join(home, "mesh-registry"));
  assert.equal(resolveRegistryDir(`~${path.sep}mesh-registry`), path.join(home, "mesh-registry"));
