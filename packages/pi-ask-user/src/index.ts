...
export default function askUserExtension(pi: ExtensionAPI) {
  pi.registerTool<typeof AskUserParams, AskUserResult>({
    ...
    label: "Ask User",
    description:
      'Ask the user one or more questions with options (single- or multi-select), for clarifying requirements, getting preferences, or confirming decisions. Reserve it for genuine decisions with a few distinct paths — options are capped at a handful, so don\'t use it to pick one item from a long list you just enumerated; present that in a normal message or just proceed with the highest-priority item. Every question always includes a built-in \'Type something.\' row (cannot be disabled), so never assume the user is restricted to your listed options. All displayed text should be in the conversation\'s language. Returns `{ cancelled, answers: [...] }`: each answer is keyed by the question\'s `tab`; fields vary by answer kind (option pick → `answer`, custom text → `custom`, multi-select picks → `answers: [...]`, skipped → `skipped: true`), and an optional user-attached `message` note may accompany the answers — treat it as high-priority context.',
